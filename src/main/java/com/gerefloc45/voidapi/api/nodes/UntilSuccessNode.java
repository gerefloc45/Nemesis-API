...
/**
 * Until success node - repeats its child behavior until it succeeds.
 * Converts FAILURE to RUNNING to continue trying.
 * 
 * @author VoidAPI Framework
 * @version 0.2.0
 */
public class UntilSuccessNode implements Behavior {
    private final Behavior child;
    private final int maxAttempts;
    private int currentAttempt;

    /**
     * Creates an until success node with unlimited attempts.
     *
     * @param child The child behavior to repeat
     */
    public UntilSuccessNode(Behavior child) {
        this(child, -1);
    }

    /**
     * Creates an until success node with a maximum number of attempts.
     *
     * @param child The child behavior to repeat
     * @param maxAttempts Maximum number of attempts (-1 for unlimited)
     */
    public UntilSuccessNode(Behavior child, int maxAttempts) {
        this.child = child;
    ...
    public Status execute(BehaviorContext context) {
        ...
        // Child failed, restart and continue
        currentAttempt++;
